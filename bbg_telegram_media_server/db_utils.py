...
def create_db(path_database: str):
    ...
    connect_db.execute('''CREATE TABLE Movie
...
def add_movie(name: str, uploaded_file_name: str, torrent_file_name: str) -> None:
    cur.execute(f'''INSERT INTO Movie (NAME, UPLOADED_FILE, TORRENT_FILE)
...
def get_movie_by_id(movie_id: int):
    res = cur.execute(f'''SELECT NAME, UPLOADED_FILE, TORRENT_FILE FROM Movie 
                    WHERE ID = {movie_id}''')
...
def login(password: str, chat_id: int, user_name: str):
    if password == config.PASSWORD:
        cur.execute(f'''INSERT INTO User (NAME, CHAT_ID)
...
def check_user(chat_id: int):
    res = cur.execute(f'''SELECT * FROM User
